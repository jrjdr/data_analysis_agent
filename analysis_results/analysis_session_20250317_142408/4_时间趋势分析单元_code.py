...
def analyze_time_trends(df):
    ...
    if 'server_name' in df.columns:
        ...
        for server in df['server_name'].unique():
            ...
            for col in ['cpu_usage_percent', 'memory_usage_percent', 'disk_usage_percent', 'network_traffic_percent']:
                if col in numeric_cols and (col, 'mean') in server_stats.columns and not pd.isna(server_stats.loc[server, (col, 'mean')]):
        ...
        results.append("未找到服务器名称列，跳过服务器性能分析。")
    ...
    for col in ['cpu_usage_percent', 'memory_usage_percent', 'disk_io_percent', 'network_traffic_percent']:
        if col in df.columns and not df[col].isna().all():
            # 创建一个临时DataFrame，只包含非NaN值
            temp_df = df.dropna(subset=[col]).copy()
            
            # 使用Z分数检测异常值
            z_scores = np.abs(stats.zscore(temp_df[col]))
            outlier_indices = np.where(z_scores > 3)[0]  # 3个标准差以外的值视为异常
            
            if len(outlier_indices) > 0:
                # 获取原始DataFrame中的对应行
                outliers = temp_df.iloc[outlier_indices]
                ...
                if len(outliers) > 0:
                    ...
                    for i in range(min(5, len(outliers))):
                        row = outliers.iloc[i]
                        server_info = f", 服务器: {row['server_name']}" if 'server_name' in row else ""
                        results.append(f"  - 时间: {row['timestamp']}{server_info}, 值: {row[col]:.2f}")
                results.append("")
    ...
    if 'event_type' in df.columns:
        ...
        results.append("\n=== 事件类型分析 ===\n")
        results.append("未找到事件类型列，跳过事件类型分析。")
    ...
    db_metrics_found = False
    for metric in db_metrics:
        if metric in df.columns and not df[metric].isna().all():
            db_metrics_found = True
    ...
    if not db_metrics_found:
        results.append("未找到数据库性能指标列，跳过数据库性能分析。")

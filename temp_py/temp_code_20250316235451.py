        ...
        with open('analysis_results.txt', 'w', encoding='utf-8') as f:
            ...
            if 'Month' in df.columns and not df['Month'].isna().all():
                ...
                if 'Date' in df.columns and pd.notna(df['Date']).any():
                    # 确保Date列已经转换为datetime类型
                    valid_dates = df[pd.notna(df['Date'])]['Date']
                    if not valid_dates.empty:
                        ...
                        f.write("无法进行月度分析：Date列中没有有效的日期数据\n\n")
                else:
            ...
            if 'Item' in df.columns and not df['Item'].isna().all():
                ...
                f.write("销量前5的商品:\n")
                for i, row in enumerate(top_items.iterrows()):
                    index, data = row
                    f.write(f"  {i+1}. {data['Item']} - 销量: {data['Sales']}, 收入: {data['Revenue']:.2f}\n")
            else:
                f.write("无法进行畅销商品分析：缺少有效的商品数据\n")

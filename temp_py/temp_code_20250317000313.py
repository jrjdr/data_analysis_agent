...
def analyze_sales_data(file_path):
    ...
    try:
        ...
        results.append(f"总销售量: {df['Sales'].sum():.0f}")  # 确保销售量为整数
        ...
        for _, row in category_stats.iterrows():
            results.append(f"类别: {row['Category']}, 销售量: {row['Sales']:.0f}, 销售额: {row['Revenue']:.2f}")
        results.append("")
        ...
        if 'Month' in df.columns and not df['Month'].isna().all():
            ...
            for _, row in month_stats.iterrows():
                results.append(f"月份: {row['Month']}, 销售量: {row['Sales']:.0f}, 销售额: {row['Revenue']:.2f}")
        else:
        ...
        if 'Week' in df.columns and not df['Week'].isna().all():
            ...
            for _, row in week_stats.iterrows():
                results.append(f"周: {row['Week']}, 销售量: {row['Sales']:.0f}, 销售额: {row['Revenue']:.2f}")
        else:
        ...
        if 'Item' in df.columns and not df['Item'].isna().all():
            ...
            for _, row in top_items.iterrows():
                results.append(f"商品: {row['Item']}, 销售量: {row['Sales']:.0f}, 销售额: {row['Revenue']:.2f}")
        else:

...
def load_and_prepare_data(file_path):
    ...
    try:
        ...
        for col in numeric_columns:
            if df[col].isna().any():
                # Forward fill and then backward fill to handle gaps
...
def analyze_time_trends(df):
    ...
    server_df_daily = server_df.resample('D').mean()
    server_df_hourly = server_df.resample('H').mean()
    ...
    for metric in metrics:
        if metric in server_df.columns and not server_df[metric].isna().all():
...
def analyze_periodicity(df):
    ...
    # Only process columns that exist and have enough data
    available_metrics = [m for m in metrics if m in df.columns and not df[m].isna().all() and len(df) > 24]
...
def analyze_correlations(df):
    ...
    available_metrics = [m for m in server_metrics if m in df.columns and not df[m].isna().all()]

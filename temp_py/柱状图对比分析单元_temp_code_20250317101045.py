...
def analyze_data(file_path):
    try:
        ...
        results['resource_block_usage_distribution'] = {str(k): int(v) for k, v in resource_block_usage_counts.items()}

        # 6. 活跃用户数量分布
        active_users_bins = pd.cut(df['active_users'], bins=5)
        active_users_counts = active_users_bins.value_counts().sort_index()
        results['active_users_distribution'] = {str(k): int(v) for k, v in active_users_counts.items()}
        ...
        results['cpu_usage_distribution'] = {str(k): int(v) for k, v in cpu_usage_counts.items()}
        ...
        results['memory_usage_distribution'] = {str(k): int(v) for k, v in memory_usage_counts.items()}

        # 9. 温度区间分布
        temperature_bins = pd.cut(df['temperature_celsius'], bins=5)
        temperature_counts = temperature_bins.value_counts().sort_index()
        results['temperature_distribution'] = {str(k): int(v) for k, v in temperature_counts.items()}
